...
export default function Features() {
  return (
    <section className="py-24 px-8">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        ...
        >
          <div className="aspect-video flex items-center justify-center">
             ...
             <Avatar className="-ml-6 border-transparent">
                <div className="size-full bg-neutral-700 rounded-full inline-flex items-center justify-center gap-2">
                    {Array.from({length: 3}).map((_, i) => (
                        <span  
                        key={i}
                        className="size-1.5 rounded-full bg-white inline-flex"></span>
                    ))}
                </div>
             </Avatar>
          </div>
      ...
      <div className="mt-8 flex flex-wrap gap-3 justify-center max-w-5xl mx-auto">
          {features.map((feature) => (
            <div key={feature} className="bg-neutral-900 border border-white/10 inline-flex px-3 md:px-5 py-1.5 md:py-2 rounded-2xl gap-3 items-center">
              <span className="bg-lime-400 text-neutral-600 size-5 rounded-full inline-flex items-center justify-center text-xl">
                &#10038;
              </span>
